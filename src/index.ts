/**
 * ClawWorker - OpenClaw on Cloudflare Sandbox
 *
 * Runs OpenClaw personal AI assistant in a Cloudflare Sandbox container.
 * Proxies requests to the gateway's web UI and WebSocket endpoint.
 *
 * Required secrets: ANTHROPIC_API_KEY (or AI Gateway), GATEWAY_TOKEN
 * Optional: TELEGRAM_BOT_TOKEN, DISCORD_BOT_TOKEN, SLACK_*
 */
...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  // CF Access vars not required in dev/test mode (auth skipped) or bootstrap mode (gateway token)
  const isBootstrapMode = !!(getGatewayToken(env) && !env.CF_ACCESS_TEAM_DOMAIN && !env.CF_ACCESS_AUD);
  if (!isTestMode && !isBootstrapMode) {
  ...
  // R2 storage is required for memory persistence (skip in dev/test)
  if (!isTestMode) {
...
app.use('*', async (c, next) => {
  ...
  const sandbox = getSandbox(c.env.Sandbox, 'clawworker', options);
