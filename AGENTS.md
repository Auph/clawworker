...
**ClawWorker** is a free, open-source way to run [OpenClaw](https://github.com/openclaw/openclaw) in your own Cloudflare Sandbox container. It provides:
...
**Always push to git immediately** after making edits. Run `git add -A && git commit -m "..." && git push` as the final step of any coding task.
...
- See `src/types.ts` for full env interface. Use `GATEWAY_TOKEN` (preferred) or legacy `MOLTBOT_GATEWAY_TOKEN`
...
- **One-step deploy:** `npm run setup` — Interactive script that installs, prompts for secrets, and deploys
- **Bootstrap mode:** When `GATEWAY_TOKEN` is set but `CF_ACCESS_*` are not, admin/API routes accept the gateway token via `?token=` or `X-Gateway-Token` header. Allows using the app without Cloudflare Access initially.
...
R2 uses rclone for backup/restore (no FUSE mount by default). Important gotchas:
